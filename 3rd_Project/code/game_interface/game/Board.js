class Board extends CGFobject {
    ...
    initBoard() {
        ...
        this.octagons[0][0].setPiece(this.sqr_material);
        this.squares[0][0].setPiece(this.oct_material);
        // to here
    }

    display() {        
        let oct_pos = this.sqr_radius + this.oct_diagonal / 2.0 + 0.0025;
        
        for (let i = 0; i < this.squares.length; i++) {
            for (let j = 0; j < this.squares[i].length; j++) {
                this.scene.pushMatrix();   
                this.scene.translate(this.sqr_radius + 0.00125 + i * (this.oct_diagonal + 0.0025), 0.05, this.sqr_radius + 0.00125 + j * (this.oct_diagonal + 0.0025));
        ...
        for (let i = 0; i < this.octagons.length; i++) {
            for (let j = 0; j < this.octagons[i].length; j++) {
                this.scene.pushMatrix();
                this.scene.translate(oct_pos + i * (this.oct_diagonal + 0.0025), 0.05, oct_pos + j * (this.oct_diagonal + 0.0025));
